	...
	public static void novoAcesso(Veiculo v) {
		...
		while (repeat1 == true) {
			try {
				...
				while (repeat == true) {
					try {
						strEntrada = JOptionPane.showInputDialog(
								"Digite a data e hora de entrada do veículo, como no exemplo:\n" + "DD/MM/AAAA HH:MM");
						...
						if (entrada.getHour() > 20 || (entrada.getHour() == 20 && entrada.getMinute() > 0)
								|| entrada.getHour() < 6)
							throw new EstacionamentoFechadoException(
									"Horário de entrada inválido, estacionamento fechado!");
						repeat = false;
					} catch (NullPointerException e) {
						String msg = "NullPointerException\nOpção cancelar indisponível quando se está fazendo o cadastro!";
						JOptionPane.showMessageDialog(null, msg);
				...
				while (repeat == true) {
					try {
						strSaida = JOptionPane.showInputDialog(
								"Digite a data e hora de saída do veículo como no exemplo:\n" + "DD/MM/AAAA HH:MM");
						if (strSaida.equals(""))
							throw new DadosAcessoIncompletosException("Campo saída vazio!");
						saida = LocalDateTime.parse(strSaida, formato);
						if (saida.getHour() > 20 || (saida.getHour() == 20 && saida.getMinute() > 0)
								|| saida.getHour() < 6)
							throw new EstacionamentoFechadoException(
									"Horário de saída inválido, estacionamento fechado!");
						repeat = false;
					} catch (NullPointerException e) {
						String msg = "NullPointerException\nOpção cancelar indisponível quando se está fazendo o cadastro!";
						;
				...
				if (saida.isBefore(entrada) == true || saida.isEqual(entrada))
					throw new PeriodoInvalidoException(
							"Período igual ou inferior a zero, digite uma data de entrada e de saída válidas!");
				repeat1 = false;
